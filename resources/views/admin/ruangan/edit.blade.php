...
@section('admin-content')
    <div class="container mx-auto p-4">
        <div class="bg-white shadow-sm border-b border-gray-200 px-8 py-4 mb-4 flex items-center gap-4">
            <h1 class="text-xl font-bold text-gray-800">
                <a href="{{ route('admin.ruangan.index') }}" class="text-black hover:text-gray-700">
                    <i class="fas fa-arrow-left"></i>
                </a>
            </h1>
            <h1 class="text-xl font-semibold text-gray-800">Edit Ruangan {{ $data->name }}</h1>
        </div>

        <div class="bg-white shadow-md rounded-lg p-6 w-fit">
            <form action="{{ route('admin.ruangan.update', $data->id) }}" method="post" class="flex flex-col gap-4 max-w-md">
                @csrf
                @method('PUT')
                @if ($errors->any())
                    ...
                    <div class="flex flex-col gap-2">
                        <div class="grid grid-cols-2 gap-4">
                            <div class="flex-1">
                                ...
                                <input type="number" name="lantai" id="lantai" value="{{ old('lantai', $lantai) }}"
                            ...
                            <div class="flex-1">
                                ...
                                <input type="number" name="no_ruangan" id="no_ruangan"
                                    value="{{ old('no_ruangan', $no_ruangan) }}"
                                    class="mt-1 block px-2 py-2 w-full border border-gray-300 rounded-md shadow-sm focus:outline-none sm:text-sm transition"
                                    placeholder="01">
                            </div>
                        </div>
    ...
    <script>
        ...
        if (@json($kode)) {
            inputKodeRuangan.value = @json($kode);
        } else {
            inputKodeRuangan.value = "JTI";
        }
    ...
    {{-- <script>
        ...
        const kodeData = @json($kode);


        const baseCode = 'JTI';

        if (kodeData && kodeData.startsWith(baseCode)) {
            const kode = kodeData;
            const remaining = kode.slice(baseCode.length); // bagian setelah 'JTI'
            let no_ruangan = '';

            if (remaining.length >= 3) {
                lantai = remaining.charAt(0);
                no_ruangan = remaining.slice(1);
            }

            document.getElementById('lantai').value = lantai;
            document.getElementById('no_ruangan').value = no_ruangan;

            document.getElementById('kode_ruangan').value = kode;
        } else {

            document.getElementById('kode_ruangan').value = baseCode;
        }
    </script> --}}
@endsection
