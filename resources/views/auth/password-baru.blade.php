<x-layout>
    <div class="flex min-h-full flex-col justify-center px-6 py-12 lg:px-8">
        ...
        <div class="top-1/2 left-1/2 transform sm:mx-auto sm:w-full sm:max-w-sm mt-6">
            <form class="space-y-4" action="{{ route('password-baru.store') }}" method="POST">
                @csrf
                @method('POST')
                @if ($errors->any())
                    <div
                    ...
                    <div x-data="{ showPassword: false }" class="flex">
                        <div class="w-5/6">
                            <input :type="showPassword ? 'text' : 'password'" name="password" id="password" required
                                class="block w-full bg-white dark:bg-gray-900 px-3 py-3 text-base text-gray-900 dark:text-gray-200 border-l-1 border-t-1 rounded-tl-md border-gray-300 dark:border-gray-700 outline-gray-300 placeholder:text-gray-600/50 dark:placeholder:text-gray-400 focus:outline-2 focus:-outline-offset-2 focus:outline-gray-800 sm:text-sm/6"
                                placeholder="Password baru">

                        </div>
                        <div
                            class="w-1/6 flex items-center justify-center bg-white dark:bg-gray-900 border-r-1 border-t-1 rounded-tr-md border-gray-300 dark:border-gray-700">
