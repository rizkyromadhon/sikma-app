...
class RekapSemesteranPerProdiSheet implements FromCollection, WithTitle, WithHeadings, WithMapping, WithEvents, WithColumnWidths, ShouldAutoSize
{
    ...
    public function map($mahasiswa): array
    {
        ...
        $jumlahHariEfektif = count($this->dateColumnsKeys);
            ...
            if ($presensiHariIni) {
                ...
                switch ($status) {
                    ...
                    case 'tidak hadir':
                        ...
                        break;
                    default:
                        $statusDisplay = '?'; // Use '?' to clearly indicate unknown statuses
                        break;
                }
            } else {
                $statusDisplay = '-';
        ...
        $rowData[] = $totalAlpha;
        $persentaseHadir = ($jumlahHariEfektif > 0) ? round(($totalHadir / $jumlahHariEfektif) * 100, 1) : 0;
    ...
    public function registerEvents(): array
    {
        return [
            AfterSheet::class => function (AfterSheet $event) {
                ...
                $sheet->getStyle('A1:' . $highestColumnLetter . $lastDataRow)->getFont()->setName('Arial'); // Changed font to Arial
                $sheet->getStyle('A1:' . $highestColumnLetter . $lastDataRow)->getFont()->setSize(10);

                // === HEADER SECTION STYLING ===

                // Baris 1: Header Institusi - Modern styling
                ...
                $sheet->getStyle('A1:' . $highestColumnLetter . '1')->getFill()->setFillType(Fill::FILL_SOLID)->getStartColor()->setARGB('2E75B6'); // Blue header
                ...
                $sheet->getStyle('A2:' . $highestColumnLetter . '2')->getFill()->setFillType(Fill::FILL_SOLID)->getStartColor()->setARGB('F2F2F2'); // Light gray
                ...
                $sheet->getStyle('A3:' . $highestColumnLetter . '3')->getFill()->setFillType(Fill::FILL_SOLID)->getStartColor()->setARGB('E7E6E6'); // Very light gray
                ...
                $sheet->getStyle('A5:B5')->getFill()->setFillType(Fill::FILL_SOLID)->getStartColor()->setARGB('2E75B6');
                ...
                $sheet->getStyle('A6:B7')->getFill()->setFillType(Fill::FILL_SOLID)->getStartColor()->setARGB('5B9BD5'); // Dark blue
                ...
                $sheet->getStyle('C6:' . $dateRangeEnd . '7')->getFill()->setFillType(Fill::FILL_SOLID)->getStartColor()->setARGB('5B9BD5'); // Medium blue
                ...
                // Day names rotation (Baris 7) - Rotated text up
                $sheet->getStyle('C7:' . $dateRangeEnd . '7')->getAlignment()->setTextRotation(90);
                $sheet->getRowDimension(6)->setRowHeight(25);
                $sheet->getRowDimension(7)->setRowHeight(50); // Increased row height for rotated text

                // === REKAPITULASI HEADERS STYLING (Baris 6 & 7) ===
                $rekapStartColIndex = 2 + $numDateColumns + 1; // Kolom Hadir
                $rekapEndColIndex = 2 + $numDateColumns + 4;   // Kolom Persentase

                // Merge cells and apply rotation for each rekap column
                $rekapHeaders = ['Hadir', 'Izin/Sakit', 'Tidak Hadir', 'Persentase'];
                foreach ($rekapHeaders as $i => $headerText) {
                    $colIndex = $rekapStartColIndex + $i;
                    $colLetter = \PhpOffice\PhpSpreadsheet\Cell\Coordinate::stringFromColumnIndex($colIndex);

                    // Merge vertical cells for each rekap header
                    $sheet->mergeCells($colLetter . '6:' . $colLetter . '7');
                    $sheet->setCellValue($colLetter . '6', $headerText); // Set value in the merged cell

                    $sheet->getStyle($colLetter . '6:' . $colLetter . '7')
                        ->getFill()->setFillType(Fill::FILL_SOLID)->getStartColor()->setARGB('00B050'); // Green
                    $sheet->getStyle($colLetter . '6:' . $colLetter . '7')
                        ->getFont()->setColor(new \PhpOffice\PhpSpreadsheet\Style\Color('FFFFFF'))->setBold(true);
                    $sheet->getStyle($colLetter . '6:' . $colLetter . '7')
                        ->getAlignment()->setHorizontal(Alignment::HORIZONTAL_CENTER)
                        ->setVertical(Alignment::VERTICAL_CENTER)
                        ->setTextRotation(90); // Rotate text up
                    $sheet->getStyle($colLetter . '6:' . $colLetter . '7')
                ...
                if ($lastDataRow >= $this->startDataRow) {
                    ...
                    $sheet->getStyle('B' . $this->startDataRow . ':B' . $lastDataRow)->getAlignment()->setHorizontal(Alignment::HORIZONTAL_LEFT); // Align nama mahasiswa to left
                    ...
                    for ($row = $this->startDataRow; $row <= $lastDataRow; $row++) {
                        if (($row - $this->startDataRow) % 2 == 0) {
                            $sheet->getStyle('A' . $row . ':' . $highestColumnLetter . $row)
                                ->getFill()->setFillType(Fill::FILL_SOLID)->getStartColor()->setARGB('F9F9F9'); // Very light gray
                        }
                    }
                    ...
                    for ($row = $this->startDataRow; $row <= $lastDataRow; $row++) {
                        for ($colIndex = 3; $colIndex <= (2 + $numDateColumns); $colIndex++) {
                            ...
                            switch ($cellValue) {
                                case '✓':
                                    $sheet->getStyle($cellCoordinate)->getFont()->setColor(new \PhpOffice\PhpSpreadsheet\Style\Color('00B050')); // Green
                                    $sheet->getStyle($cellCoordinate)->getFont()->setBold(true)->setSize(12);
                                    $sheet->getStyle($cellCoordinate)->getFill()->setFillType(Fill::FILL_SOLID)->getStartColor()->setARGB('9BFFC8');
                                    break;
                                case '✗':
                                    $sheet->getStyle($cellCoordinate)->getFont()->setColor(new \PhpOffice\PhpSpreadsheet\Style\Color('FF0000')); // Red
                                    $sheet->getStyle($cellCoordinate)->getFont()->setBold(true)->setSize(12);
                                    $sheet->getStyle($cellCoordinate)->getFill()->setFillType(Fill::FILL_SOLID)->getStartColor()->setARGB('FFB9B9');
                                    break;
                                case 'I':
                                    $sheet->getStyle($cellCoordinate)->getFont()->setColor(new \PhpOffice\PhpSpreadsheet\Style\Color('FF8C00')); // Orange
    ...
    private function styleMonthHeader($sheet, $range)
    {
        ...
        $sheet->getStyle($range)->getFill()->setFillType(Fill::FILL_SOLID)->getStartColor()->setARGB('2E75B6'); // Blue
    ...
    private function styleSummaryHeader($sheet, $range)
    {
        $sheet->getStyle($range)->getAlignment()->setHorizontal(Alignment::HORIZONTAL_CENTER)->setVertical(Alignment::VERTICAL_CENTER);
        $sheet->getStyle($range)->getFont()->setBold(true)->setSize(11)->setColor(new \PhpOffice\PhpSpreadsheet\Style\Color('FFFFFF'));
        $sheet->getStyle($range)->getFill()->setFillType(Fill::FILL_SOLID)->getStartColor()->setARGB('00B050'); // Green
        $sheet->getStyle($range)->getBorders()->getAllBorders()->setBorderStyle(Border::BORDER_MEDIUM)->getColor()->setARGB('FFFFFF');
    }
}
